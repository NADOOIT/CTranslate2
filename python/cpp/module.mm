...
std::vector<std::string> list_supported_devices() {
  std::vector<std::string> devices;
  devices.emplace_back("cpu");
#ifdef CT2_WITH_CUDA
  if (ctranslate2::cuda::has_gpu())
    devices.emplace_back("cuda");
#endif
#ifdef CT2_WITH_METAL
  if (ctranslate2::metal::has_metal())
    devices.emplace_back("mps"); // Report as 'mps' to align with user expectation
#endif
  return devices;
}
...
PYBIND11_MODULE(_ext, m) {
  m.def("check_metal_directly", &ctranslate2::metal::has_metal, "Directly check Metal availability via ctranslate2::metal::has_metal");
  ...
  m.def("list_supported_devices", &list_supported_devices, R"doc(
        Returns a list of supported compute devices based on the build configuration.

        Returns:
            A list of strings, e.g., ["cpu", "cuda", "mps"].
      )doc");
