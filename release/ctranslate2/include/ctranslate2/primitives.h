...
#include "ctranslate2/storage_view.h" // Ensure StorageView is known
...
namespace ctranslate2 {
  ...
  // Specialization for Metal device
  template <>
  struct primitives<Device::METAL> : primitives<Device::CPU> {
    static const Device device = Device::METAL;
    ...
    static void attention(
        ...
        const StorageView* bias,         // Can be used for attention mask
        const StorageView* key_lengths,  // Optional: for sequences of varying lengths
        const StorageView* query_lengths,// Optional: for sequences of varying lengths
        const float scale,
        const int num_heads,
        const bool causal
    );

    // Declare other Metal-specific primitive specializations here if any.
    // For example:
    // template <typename T>
    // static void add(const T* a, const T* b, T* c, dim_t size);
  };
