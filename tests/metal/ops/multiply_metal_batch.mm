...
extern "C" bool multiply_metal_batch(const std::vector<const float*>& a, const std::vector<const float*>& b, std::vector<float*>& c, int size) {
    @autoreleasepool {
        ...
        if (!device) {
            std::cerr << "[MULTIPLY-Metal-Batch] Kein Metal-Device gefunden!" << std::endl;
            return false;
        }
        ...
        if (!lib) {
            NSString* path = @"multiply_kernel.metallib";
            NSFileManager* fm = [NSFileManager defaultManager];
            if (![fm fileExistsAtPath:path]) {
                NSString* exePath = [[NSBundle mainBundle] executablePath];
                NSString* exeDir = [exePath stringByDeletingLastPathComponent];
                path = [exeDir stringByAppendingPathComponent:@"multiply_kernel.metallib"];
            }
            NSError* libErr = nil;
            lib = [device newLibraryWithFile:path error:&libErr];
            if (!lib) {
                std::cerr << "[MULTIPLY-Metal-Batch] Kein Metal-Library!" << std::endl;
                return false;
            }
        }
        id<MTLFunction> func = [lib newFunctionWithName:@"multiply_kernel"];
        if (!func) {
            std::cerr << "[MULTIPLY-Metal-Batch] Kein multiply_kernel!" << std::endl;
            return false;
        }
        id<MTLComputePipelineState> pipeline = [device newComputePipelineStateWithFunction:func error:&err];
        if (!pipeline) {
            std::cerr << "[MULTIPLY-Metal-Batch] Pipeline-Fehler!" << std::endl;
            return false;
        }
