...
void add_adaptive(const float* a, const float* b, float* c, int size) {
    if (size < 4096) {
        add_cpu(a, b, c, size);
    } else if (size < 65536) {
        add_cpu_mt(a, b, c, size);
    } else {
        std::vector<const float*> batch_a = {a};
        std::vector<const float*> batch_b = {b};
        std::vector<float*> batch_c = {c};
        add_metal_batch(batch_a, batch_b, batch_c, size);
    }
}

}

TEST(AddAdaptiveDispatch, SmallArrayCPUSingle) {
    int size = 128;
    std::vector<float> a(size), b(size), c(size), ref(size);
    std::iota(a.begin(), a.end(), 1.0f);
    std::iota(b.begin(), b.end(), -64.0f);
    add_dispatch::add_adaptive(a.data(), b.data(), c.data(), size);
    add_dispatch::add_cpu(a.data(), b.data(), ref.data(), size);
    for (int i = 0; i < size; ++i)
        ASSERT_FLOAT_EQ(c[i], ref[i]);
}

TEST(AddAdaptiveDispatch, MediumArrayCPUMulti) {
    int size = 8192;
    std::vector<float> a(size), b(size), c(size), ref(size);
    std::iota(a.begin(), a.end(), 1.0f);
...
TEST(AddAdaptiveDispatch, LargeArrayMetalBatch) {
    int size = 131072;
    std::vector<float> a(size), b(size), c(size), ref(size);
    std::iota(a.begin(), a.end(), 1.0f);
...
TEST(AddAdaptiveDispatch, BatchedDispatchCorrectness) {
    ...
    for (int b = 0; b < batch; ++b) {
        ...
        ptr_b[b] = bs[b].data();
        ptr_c[b] = cs[b].data();
        add_dispatch::add_cpu(as[b].data(), bs[b].data(), refs[b].data(), size);
    }
    ASSERT_TRUE(add_dispatch::add_metal_batch(ptr_a, ptr_b, ptr_c, size));
    for (int b = 0; b < batch; ++b)
        for (int i = 0; i < size; ++i)
