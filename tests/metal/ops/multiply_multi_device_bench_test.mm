...
void bench_multiply(const std::string& mode, int size, int batch, std::ofstream& csv) {
    ...
    for (int r = 0; r < repeat; ++r) {
        ...
        if (mode == "CPU_SINGLE") {
            for (int b = 0; b < batch; ++b)
                multiply_dispatch::multiply_cpu(as[b].data(), bs[b].data(), cs[b].data(), size);
        } else if (mode == "CPU_MULTI") {
            for (int b = 0; b < batch; ++b)
                multiply_dispatch::multiply_cpu_mt(as[b].data(), bs[b].data(), cs[b].data(), size);
...
int main() {
    std::ofstream csv("benchmarks_multiply_dispatch.csv");
    csv << "mode,size,batch,avg_ms,latency,throughput" << std::endl;
    std::vector<int> sizes = {128, 1024, 8192, 32768, 131072, 524288};
    std::vector<int> batches = {1, 8, 32, 128};
    for (auto size : sizes) {
        for (auto batch : batches) {
            bench_multiply("CPU_SINGLE", size, batch, csv);
            bench_multiply("CPU_MULTI", size, batch, csv);
            bench_multiply("METAL_BATCH", size, batch, csv);
        }
    }
    return 0;
}
